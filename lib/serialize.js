...
export function Client({ hinata, store }) {
   ...
   const client = Object.defineProperties(hinata, {
      getContentType: {
         value(content) {
            if (content) {
      ...
      getName: {
         value(jid) {
            let id = hinata.decodeJid(jid)
            let v
            if (id?.endsWith("@g.us")) return new Promise(async (resolve) => {
               v = hinata.contacts[id] || hinata.messages["status@broadcast"]?.array?.find(a => a?.key?.participant === id)
               ...
               hinata.user : (hinata.contacts[id] || {})
      ...
      sendContact: {
         async value(jid, number, quoted, options = {}) {
            ...
            for (let v of number) {
               list.push({
                  displayName: await hinata.getName(v),
                  vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await hinata.getName(v + "@s.whatsapp.net")}\nFN:${await hinata.getName(v + "@s.whatsapp.net")}\nitem1.TEL;waid=${v}:${v}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:${setting.Exif.packEmail}\nitem2.X-ABLabel:Email\nitem3.URL:${setting.Exif.packWebsite}\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
               })
            }
            return hinata.sendMessage(jid, {
               contacts: {
                  displayName: `${list.length} Contact`,
                  contacts: list
               },
               mentions: quoted?.participant ? [hinata.decodeJid(quoted?.participant)] : [hinata.decodeJid(hinata?.user?.id)],
               ...options
            }, { quoted, ...options })
         },
         enumerable: true
      ...
      sendMedia: {
         async value(jid, url, quoted = "", options = {}) {
            ...
            if (size > 45000000) data = { document: buffer, mimetype: mime, fileName: options?.fileName ? options.fileName : `${hinata.user?.name} (${new Date()}).${ext}`, ...options }
            else if (options.asDocument) data = { document: buffer, mimetype: mime, fileName: options?.fileName ? options.fileName : `${hinata.user?.name} (${new Date()}).${ext}`, ...options }
            ...
            let msg = await hinata.sendMessage(jid, data, { quoted, ...options })
            return msg
         },
         enumerable: true
      },
      ...
      cMod: {
         value(jid, copy, text = '', sender = hinata.user.id, options = {}) {
            let mtype = hinata.getContentType(copy.message)
            ...
            copy.key.fromMe = areJidsSameUser(sender, hinata.user.id)
      ...
      sendPoll: {
         async value(chatId, name, values, options = {}) {
            ...
            return await hinata.sendMessage(chatId, {
               poll: {
                  name,
                  values,
                  selectableCount
      ...
      sendGroupV4Invite: {
         async value(jid, groupJid, inviteCode, inviteExpiration, groupName, jpegThumbnail, caption = "Invitation to join my WhatsApp Group", options = {}) {
            const media = await prepareWAMessageMedia({ image: (await Function.getFile(jpegThumbnail)).data }, { upload: hinata.waUploadToServer })
            ...
            const m = generateWAMessageFromContent(jid, message, { userJid: hinata.user?.id })
            await hinata.relayMessage(jid, m.message, { messageId: m.key.id })
...
export async function Serialize(hinata, msg) {
   const m = {}
   const botNumber = hinata.decodeJid(hinata.user?.id)
   ...
   if (msg.key) {
      m.key = msg.key
      m.from = hinata.decodeJid(m.key.remoteJid)
      ...
      m.sender = hinata.decodeJid(m.fromMe ? hinata.user.id : m.isGroup ? m.participant : m.from)
   }

   m.pushName = msg.pushName
   m.isOwner = m.sender && [...setting.options.owner, botNumber.split`@`[0]].includes(m.sender.replace(/\D+/g, ""))
   if (m.isGroup) {
      m.metadata = await hinata.groupMetadata(m.from)
   ...
   if (m.message) {
      m.type = hinata.getContentType(m.message) || Object.keys(m.message)[0]
      ...
      m.prefix = setting.options.prefix.test(m.body) ? m.body.match(setting.options.prefix)[0] : "#"
      ...
      m.reply = async (text, options = {}) => {
         ...
         if ((Buffer.isBuffer(text) || /^data:.?\/.*?;base64,/i.test(text) || /^https?:\/\//.test(text) || fs.existsSync(text))) {
            ...
            if (!options.mimetype && (/utf-8|json/i.test(data.mime) || data.ext == ".bin" || !data.ext)) {
               if (!!setting.msg[text]) text = setting.msg[text]
               return hinata.sendMessage(chatId, { text, mentions: [m.sender, ...hinata.parseMention(text)], ...options }, { quoted, ephemeralExpiration: m.expiration, ...options })
            } else {
               return hinata.sendMedia(m.from, data.data, quoted, { ephemeralExpiration: m.expiration, ...options })
            }
         } else {
            if (!!setting.msg[text]) text = setting.msg[text]
            return hinata.sendMessage(chatId, { text, mentions: [m.sender, ...hinata.parseMention(text)], ...options, }, { quoted, ephemeralExpiration: m.expiration, ...options });
      ...
      m.download = (filepath) => {
         if (filepath) return hinata.downloadMediaMessage(m, filepath)
         else return hinata.downloadMediaMessage(m)
      }
   }
   ...
   if (m.msg?.contextInfo?.quotedMessage) {
      ...
      if (m.quoted.message) {
         m.quoted.type = hinata.getContentType(m.quoted.message) || Object.keys(m.quoted.message)[0]
         m.quoted.msg = extractMessageContent(m.quoted.message[m.quoted.type]) || m.quoted.message[m.quoted.type]
         m.quoted.key = {
            remoteJid: m.msg?.contextInfo?.remoteJid || m.from,
            participant: m.msg?.contextInfo?.remoteJid?.endsWith("g.us") ? hinata.decodeJid(m.msg?.contextInfo?.participant) : false,
            fromMe: areJidsSameUser(hinata.decodeJid(m.msg?.contextInfo?.participant), hinata.decodeJid(hinata?.user?.id)),
         ...
         m.quoted.sender = hinata.decodeJid(m.msg?.contextInfo?.participant)

         m.quoted.isOwner = m.quoted.sender && [...setting.options.owner, botNumber.split`@`[0]].includes(m.quoted.sender.replace(/\D+/g, ""))
         if (m.quoted.isGroup) {
            m.quoted.metadata = await hinata.groupMetadata(m.quoted.from)
         ...
         m.quoted.prefix = setting.options.prefix.test(m.quoted.body) ? m.quoted.body.match(setting.options.prefix)[0] : "#"
         ...
         m.quoted.download = (filepath) => {
            if (filepath) return hinata.downloadMediaMessage(m.quoted, filepath)
            else return hinata.downloadMediaMessage(m.quoted)
         }
      }
   }
